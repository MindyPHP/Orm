...
class Issue64Test extends OrmDatabaseTestCase
{
    ...
    public function testIssue64()
    {
        ...
        $this->assertEquals("SELECT `product_1`.* FROM `product` `product_1` LEFT OUTER JOIN `product_product_list` `product_product_list_2` ON `product_1`.`id` = `product_product_list_2`.`product_id` LEFT OUTER JOIN `product_list` `product_list_3` ON `product_product_list_2`.`product_list_id` = `product_list_3`.`id` WHERE ((`product_1`.`category_id`='1')) AND ((`product_list_3`.`name`='asd')) GROUP BY `product_1`.`id`",
